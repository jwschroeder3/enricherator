...
option_list = list(
    ...
    make_option(
        c("--bayes_factor_files"), type="character", default=NULL,
        help="Name of the bedgraph files containing the Bayes factors to be used in setting opacity of the plotted genome positions. Must be in same order as files in --infiles",
    ),
    make_option(
...
for (i in 1:length(infiles)) {
    ...
    if (!is.null(opt$bayes_factor_files)) {
        if (is.null(k_files)) {
            k_files = str_split(opt$bayes_factor_files, ",", simplify=TRUE)[1,]
        }
        if (length(infiles) != length(k_files)) {
            stop("The number of input files must match the number of bayes factor files")
        }
        this_k_file = k_files[i]
        tmp_k = read_table(
            file=this_k_file,
            col_names=colnames,
            col_types=coltypes,
