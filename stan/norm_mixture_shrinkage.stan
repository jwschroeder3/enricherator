...
transformed parameters {
    ...
    for (b in 1:B) {
        for (q in 1:Q) {
            lprior += beta_lpdf(p_slab[b,q] | 0.01, 0.20);
            lprior += inv_gamma_lpdf(tausq[b,q] | 0.5, 0.5);
            lprior += normal_lpdf(slab[b,q] | 0, tausq[b,q]);
            lprior += normal_lpdf(spike[b,q] | 0, 0.01);;
