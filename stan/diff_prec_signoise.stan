...
parameters {
    array[2,Q] vector<lower=0>[L] prec; // stratify global precision inference by extracted vs input
    ...
    //array[S,Q] vector<lower=0, upper=1>[L] sig_noise; // signal-to-noise allocation for each sample, at each position
    array[S,Q] vector<lower=0, upper=1>[b_sub_L] sub_sig_noise; // one intercept for each genotype/sub-position
...
transformed parameters {
    ...
    for (s in 1:S) {
        for (q in 1:Q) {
            lprior += beta_lpdf(sub_sig_noise[s,q] | 1, 1);
            sig_noise[s,q] = csr_matrix_times_vector(
                L,
                b_sub_L,
                b_weights_vals,
                b_col_accessor,
    ...
    for (t in 1:2) {
        for (q in 1:Q) {
            lprior += lognormal_lpdf(prec[t,q] | 0, 2);
        }
    }
    lprior += student_t_lpdf(Gamma | 3, 0, 5);
    lprior += normal_lpdf(wsh | 0, 3);
}
...
model {
    ...
    real wsh_s;
    real libsize_s;
    vector[L] log_signoise;
    vector[L] log_comp_signoise;
    vector[L] prec_tq;
    ...
    for (s in 1:S) {
        ...
        for (q in 1:Q) {
            log_signoise = log(sig_noise[s,q]);
            log_comp_signoise = log1m(sig_noise[s,q]);
            prec_tq = prec[sample_type+1,q];
            Y_hat_signal = Alpha[genotype,q]
                + sample_type * Beta[genotype,q]
                + libsize_s;
            //Y_hat_noise = wsh_s + libsize_s;
//////////////// Stop the mixing with weights, just add the likelihoods for joint model.
//////////////// Last ditch thing to try; BS inflation. At every point, decide whether data is from background vs signal
            Y_hat_noise = wsh_s + libsize_s;

            for (l in 1:L) {

                // allocates reads proportionally to signal/noise
                target += log_sum_exp(
                    log_signoise[l] + neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_signal[l], prec_tq[l]),
                    log_comp_signoise[l] + poisson_log_lpmf(Y[s,q,l] | Y_hat_noise)
                );
            }
...
generated quantities {
    ...
    //                for (l in 1:L) {
    //                    if (Y[s,q,l] < 5) {
    //                        post_pred[s,q][l] = neg_binomial_2_rng(exp(Y_hat_sq[l]), low_prec[sample_type+1]);
    //                        log_lik[n] = neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_sq[l], low_prec[sample_type+1]);
    //                    } else {
