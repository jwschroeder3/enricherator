...
parameters {
    ...
    vector<lower=0, upper=1>[2] zi; // zero inflation for input and chip
...
transformed parameters {
    ...
    // prior on being in signal fraction conditional on not being zi fraction
    lprior += beta_lpdf(sig_noise | 25.0, 1.0);
    // prior on being in zi fraction
    lprior += beta_lpdf(zi | 1.0, 10.0);
}
...
model {
    ...
    vector[2] log_zi = log(zi);
    vector[2] log_comp_zi = log1m(zi);
    ...
    for (s in 1:S) {
        ...
        log_zi_s = log_zi[sample_type + 1];
        log_comp_zi_s = log_comp_zi[sample_type + 1];
        genotype = geno_x[s];
        if (shared_input) {
            alpha_genotype = 1;
        } else {
        ...
        for (q in 1:Q) {
            ...
            for (l in 1:L) {
                ...
                if (Y[s,q,l] == 0) {
                    // allocates counts proportionally to signal/noise and zi,
                    // where noise is just a fraction of libsize
                    target += log_sum_exp(
                        [log_zi_s,
                        log_comp_zi_s + log_signoise_s + neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_signal[l], prec[sample_type+1]),
                        log_comp_zi_s + log_comp_signoise_s + poisson_log_lpmf(Y[s,q,l] | libsize_s)]
                    );
                } else {
