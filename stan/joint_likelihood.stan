...
parameters {
    ...
    array[Q] vector[S] wsh; // wsh term for how much wsh for a given sample
...
transformed parameters {
    ...
    lprior += lognormal_lpdf(prec | 0, 2);
    lprior += student_t_lpdf(Gamma | 3, 0, 5);
    for (q in 1:Q) {
        lprior += normal_lpdf(wsh[q] | -2, 1);
    }
}
...
model {
    ...
    for (s in 1:S) {
        ...
        for (q in 1:Q) {
            //prec_t = prec[sample_type+1];
            Y_hat_signal = Alpha[genotype,q]
                + sample_type * Beta[genotype,q]
                + libsize_s;
            Y_hat_noise = wsh[q][s] + libsize_s;
            //Y_sq = Y[s,q];

            //target += neg_binomial_2_log_lpmf(Y[s,q] | Y_hat_signal, prec[sample_type+1]);

            for (l in 1:L) {

                // allocates reads proportionally to signal/noise
                target += log_sum_exp(
                    neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_signal[l], prec[sample_type+1]),
                    poisson_log_lpmf(Y[s,q,l] | Y_hat_noise)
                );
            }
