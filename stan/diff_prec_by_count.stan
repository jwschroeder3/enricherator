...
parameters {
    ...
    vector<lower=0>[2] low_prec; // stratify global precision inference by extracted vs input
    //vector<lower=0>[S] wsh; // wsh parameter for sample-wise global noise
...
model {
    ...
    int genotype;
    vector[L] Y_hat_sq;
    //vector[L] disp_hat_sq;

    target += lprior;

    prec ~ cauchy(0, 1);
    low_prec ~ cauchy(0, 0.01);
    for (s in 1:S) {
        ...
        for (q in 1:Q) {
            ...
            for (l in 1:L) {
                if (Y[s,q,l] < 5) {
                    target += neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_sq[l], low_prec[sample_type+1]);
                } else {
                    target += neg_binomial_2_log_lpmf(Y[s,q,l] | Y_hat_sq[l], prec[sample_type+1]);
                }
            }
        }
    }
